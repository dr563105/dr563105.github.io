...
```{.yaml filename="change event data stream"}
{
  "value":{
    "before":null,
    "after":{
...
<!--# Quarto note - Using .yaml is just for syntax highlighting as {.json} doesn't recognise code annotations.-->
```{.yaml}  
{
  "value":{
    "before":{
...
```{.yaml filename="pg-src-connector.json"}
{
