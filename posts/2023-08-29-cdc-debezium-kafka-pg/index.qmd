...
The configuration can be supplied as data argument in a `curl` command.
`JSON` files don't allow for secure transfer of environment variables. So will put the `json` connector config in the `data` directly inside a shell script. This way sensitive properties can still use reference to the env variables.
