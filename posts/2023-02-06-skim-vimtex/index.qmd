...
# toc: true
# layout: post
image: neovim-logo1.png #320px-Neovim-logo.png #neovim-logo.png 
...
PDF readers allow easy integration with LaTeX. Since Zathura's installation in Mac OS involves more steps, 
...
```
1. Choose which program to use to view PDF file.
2. Value 1 allows forward search after every successful compilation.
3. Value 1 allows change focus to skim after command `:VimtexView` is given.
...
::: callout-important
Skim must be started by VimTeX (either through compiler callback or explicitly via <leader>lv) 
